...
function getSilverStar(input: string, elfPower: number = 3, noElfShouldDie = false): [boolean, number, number, number] {
  ...
  const attack = (unit: Unit) => {
    ...
    dx.forEach((_, i) => {
      ...
      if (candidate && candidate.type !== type && (!enemy || candidate.hp < enemy.hp)) {
        enemy = candidate;
      }
    });
  ...
  while (!combatEnded) {
    ...
    if (noElfShouldDie && units.find(({ hp, type }) => hp === 0 && type === E)) {
      return [false, -1, -1, -1];
    }

    units = units.filter(({ hp }) => hp > 0).sort((a, b) => (a.y === b.y ? a.x - b.x : a.y - b.y));
  }
